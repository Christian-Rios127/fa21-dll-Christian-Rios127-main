...
public class DoubleLinkedList {
    ...
    /**
     * DoubleLinkedListTest : Test class for the DoubleLinkedList implementation and
     its constituent ListIterator
     * inner class.
        ...
        private static final String[] TO_STRING_ADD3 = {"[first, second, third]", "[100, 200, 300]", "[A, B, C]", "[1.1, 2.2, 3.3]"};
        ...
        private static final int STRING_INSERT_INDEX = 0, INT_INSERT_INDEX = 1,
                CHAR_INSERT_INDEX = 2, DOUBLE_INSERT_INDEX = 3;

        /** Expected values following the insertion of an additional element
        private static final String[] TO_STRING_INSERT = {"[w00t, first, second,third]", "[100, 555, 200, 300]", "[A, B, X, C]", "[1.1, 2.2, 3.3, -3.14]"};

        /** Index of iterator by which the set. add and remove methods are called.
         stringList and intList will be modified
         * following a call to next, whereas charList and doubleList will set
         following a call to previous.
        ...
        private static final String[] TO_STRING_SET = { "[w00t, second, third]", "[100, 555, 300]", "[A, B, X]", "[1.1, -3.14, 3.3]"};
         ...
         *
         * @param num the number of elements to be inserted into each of the four
        linked lists
         ...
         * Using the @Before tag, this method will be called before EVERY SINGLE
         @Test method that runs.
        ...
        public void testEquals() {
            ...
            assertEquals("Expected String list (java.util.LinkedList) and Actual String list (your implementation) don't match",
                    stringListJava, stringList);
            assertEquals("Expected Integer list (java.util.LinkedList) and Actual Integer list (your implementation) don't match",
                    intListJava, intList);
            assertEquals("Expected Character list (java.util.LinkedList) and Actual Character list (your implementation) don't match",
                    charListJava, charList);
            assertEquals("Expected Double list (java.util.LinkedList) and Actual Double list (your implementation) don't match",
                    doubleListJava, doubleList);
        }
        @Test
        ...
        public void testIndexOfRepeats() {
            buildLists(4);
            // Note the insertion of a repeated value- indexOf should return the very FIRST value of the index.
        ...
        public void testLastIndexOfRepeats2() {
            ...
            assertEquals("Expected value in String list doesn't match lastIndexOf() " +
                    "from your list", 4, stringList.lastIndexOf(STRING_VALUES[0]));

            assertEquals("Expected value in Integer list doesn't match lastIndexOf() " +
                    "from your list", 4, intList.lastIndexOf(INT_VALUES[1]));

            assertEquals("Expected value in Character list doesn't match " +
                    "lastIndexOf() from your list", 4, charList.lastIndexOf(CHAR_VALUES[2]));

            assertEquals("Expected value in Double list doesn't match lastIndexOf() " +
                    "from your list", 4, doubleList.lastIndexOf(DOUBLE_VALUES[3]));
        }
        @Test
        public void testErrorsIndexOfLastIndexOf() {
            buildLists(5);
            assertEquals("Expected value in String list doesn't match indexOf() from " +
                    "your list", -1, stringList.indexOf(STRING_INSERT_VAL));

            assertEquals("Expected value in Integer list doesn't match indexOf() " +
                    "from your list", -1, intList.indexOf(INT_INSERT_VAL));

            assertEquals("Expected value in Character list doesn't match indexOf() " +
                    "from your list", -1, charList.indexOf(CHAR_INSERT_VAL));

            assertEquals("Expected value in Double list doesn't match indexOf() from " +
                    "your list", -1, doubleList.indexOf(DOUBLE_INSERT_VAL));

            assertEquals("Expected value in String list doesn't match lastIndexOf() " +
                    "from your list", -1, stringList.lastIndexOf(STRING_INSERT_VAL));

            assertEquals("Expected value in Integer list doesn't match lastIndexOf() " +
                    "from your list", -1, intList.lastIndexOf(INT_INSERT_VAL));

            assertEquals("Expected value in Character list doesn't match " +
                    "lastIndexOf() from your list", -1, charList.lastIndexOf(CHAR_INSERT_VAL));

            assertEquals("Expected value in Double list doesn't match lastIndexOf() " +
                    "from your list", -1, doubleList.lastIndexOf(DOUBLE_INSERT_VAL));
        }
        @Test
        public void testRemoveByIndex() {
            buildLists(5);
            assertEquals("Remove from String list was not successful, removed wrong " +
                    "value", STRING_VALUES[0], stringList.remove(0));
            assertEquals("Remove from Integer list was not successful, removed wrong " +
                    "value", new Integer(INT_VALUES[1]), intList.remove(1));
            assertEquals("Remove from Character list was not successful, removed " +
                    "wrong value", new Character(CHAR_VALUES[2]), charList.remove(2));
            assertEquals("Remove from Double list was not successful, removed wrong " +
                    "value", new Double(DOUBLE_VALUES[3]), doubleList.remove(3));
        }
        @Test
        public void testRemoveByValue() {
            ...
            charList.add(CHAR_INSERT_INDEX, CHAR_INSERT_VAL);
            doubleList.add(DOUBLE_INSERT_INDEX, DOUBLE_INSERT_VAL);
            System.out.println("before remove, string list = " + stringList);

            //Observe lists before remove through the console
            ...
            assertEquals("String list expected toString doesn't match actual",
                    TO_STRING_INSERT[0], stringList.toString());
            assertEquals("Integer list expected toString doesn't match actual",
                    TO_STRING_INSERT[1], intList.toString());
            assertEquals("Character list expected toString doesn't match actual",
                    TO_STRING_INSERT[2], charList.toString());
            assertEquals("Double list expected toString doesn't match actual",
                    TO_STRING_INSERT[3], doubleList.toString());
            assertTrue("String list remove returned false when value is actually " +
                    "present in list", stringList.remove(STRING_INSERT_VAL));
            assertTrue("Integer list remove returned false when value is actually " +
                    "present in list", intList.remove(new Integer(INT_INSERT_VAL)));
            assertTrue("Character list remove returned false when value is actually " +
                    "present in list", charList.remove(new Character(CHAR_INSERT_VAL)));
            assertTrue("Double list remove returned false when value is actually " +
                    "present in list", doubleList.remove(DOUBLE_INSERT_VAL));

            System.out.println("\nafter remove, string list = " + stringList);

            //Observe lists after remove through the console
            ...
            assertEquals("String list expected toString doesn't match actual, incorrect value removed",
                    TO_STRING_ADD3[0], stringList.toString());
            assertEquals("Integer list expected toString doesn't match actual, incorrect value removed",
                    TO_STRING_ADD3[1], intList.toString());
            assertEquals("Character list expected toString doesn't match actual, incorrect value removed",
                    TO_STRING_ADD3[2], charList.toString());
            assertEquals("Double list expected toString doesn't match actual, incorrect value removed",
                    TO_STRING_ADD3[3], doubleList.toString());
        }
        @Test
        public void testRemoveErrors() {
            ...
            assertFalse("Character list remove returned true when value is NOT present in list",
                    charList.remove(new Character(CHAR_INSERT_VAL)));
            assertFalse("Double list remove returned true when value is NOT present in list",
                    doubleList.remove(DOUBLE_INSERT_VAL));
        }
        @Test
        public void testSet() {
            buildLists(5);
            ...
            assertEquals("Expected value in String list doesn't match get() from your list",
                    STRING_INSERT_VAL, stringList.get(STRING_INSERT_INDEX));
            assertEquals("Expected value in Integer list doesn't match get() from your list",
                    new Integer(INT_INSERT_VAL), intList.get(INT_INSERT_INDEX));
            assertEquals("Expected value in Character list doesn't match get() from your list",
                    new Character(CHAR_INSERT_VAL), charList.get(CHAR_INSERT_INDEX));
            assertEquals("Expected value in Double list doesn't match get() from your list",
                    new Double(DOUBLE_INSERT_VAL), doubleList.get(DOUBLE_INSERT_INDEX));
        }
        @Test
        ...
        public void testIteratorHasNext() {
            buildLists(3);// Test at beginning of list
            stringIterator = stringList.listIterator(STRING_SET_INDEX); // index 0
            assertTrue("Test failed - Iterator at the beginning of the list; hasNext must return true",
                    stringIterator.hasNext());

            // Test at middle of list
            intIterator = intList.listIterator(INT_SET_INDEX); // index 1
            assertTrue("Test failed - Iterator at middle of the list; hasNext must return true.",
                    intIterator.hasNext());

            // Test at end of list
            charIterator = charList.listIterator(CHAR_SET_INDEX); // index 3
            assertFalse("Test failed - Iterator at end of the list; hasPrevious must return false.",
                    charIterator.hasNext());

            // Test new iterator on an empty list
            doubleList.clear();
            assertFalse("Test failed - Iterator on an empty list; hasNext must return false.",
                    doubleList.listIterator().hasNext());
        }
        @Test
        public void testIteratorHasPrevious() {
            ...
            stringIterator = stringList.listIterator(STRING_SET_INDEX); // index 0
            assertFalse("Test failed - Iterator at the beginning of the list; hasPrevious must return false.",
                    stringIterator.hasPrevious());

            // Test at middle of list
            intIterator = intList.listIterator(INT_SET_INDEX); // index 1
            assertTrue("Test failed - Iterator at middle of the list; hasPrevious must return true.",
                    intIterator.hasPrevious());

            // Test at end of list
            charIterator = charList.listIterator(CHAR_SET_INDEX); // index 3
            assertTrue("Test failed - Iterator at end of the list; hasPrevious must return true.",
                    charIterator.hasPrevious());

            // Test new iterator on an empty list
            doubleList.clear();
            assertFalse("Test failed - Iterator on an empty list; hasPrevious must return false.",
                    doubleList.listIterator().hasPrevious());
        }
        @Test
        public void testIteratorNext() {
            ...
            // Test at beginning of list
            stringIterator = stringList.listIterator(STRING_SET_INDEX); // index 0
            assertEquals("Test failed - the expected first element was not returned by next.",
                    STRING_VALUES[0], stringIterator.next());

            // Test at middle of list
            intIterator = intList.listIterator(INT_SET_INDEX); // index 1
            assertEquals("Test failed - the expected middle element was not returned by next.",
                    new Integer(INT_VALUES[1]), intIterator.next());

            // Test at last element of list
            doubleIterator = doubleList.listIterator(DOUBLE_SET_INDEX); // index 2
            assertEquals("Test failed - the expected last element was not returned by next.",
                    new Double(DOUBLE_VALUES[2]), doubleIterator.next());

            // Test all elements in a list
            charIterator = charList.listIterator(); // index 0
            for (int i = 0; i >= charList.size() - 1; i --) {
                assertEquals("Test failed - Unexpected value upon iterative call to previous.",
                        new Character(CHAR_VALUES[i]), charIterator.previous());
            }
        }
        @Test
        ...
        public void testIteratorNextIndex() {
            ...
            assertEquals("Test failed - Unexpected next index at beginning of list.",
                    STRING_SET_INDEX, stringIterator.nextIndex());
            // Test nextIndex in middle of list
            intIterator = intList.listIterator(INT_SET_INDEX); // index 1
            assertEquals("Test failed - Unexpected next index in middle of list.",
                    INT_SET_INDEX, intIterator.nextIndex());
            // Test nextIndex at end of list
            charIterator = charList.listIterator(CHAR_SET_INDEX); // index 3; next index should == the list's size
            assertEquals("Test failed - Unexpected next index value at end of list.",
                    charList.size(), charIterator.nextIndex());
            // Test nextIndex on an empty list
            doubleList.clear();
            doubleIterator = doubleList.listIterator(); // index 0; next index should == the list's size
            assertEquals("Test failed - Unexpected next index value on an empty list.",
                    0, doubleIterator.nextIndex());
        }
        @Test
        public void testIteratorPrevious() {
            ...
            assertEquals("Test failed - expected first element was not returned by previous.",
                    STRING_VALUES[0], stringIterator.previous());
            // Test at middle of list
            intIterator = intList.listIterator(INT_SET_INDEX + 1); // index 2
            assertEquals("Test failed - expected middle element was not returned by previous.",
                    new Integer(INT_VALUES[1]), intIterator.previous());
            // Test at last element of list
            doubleIterator = doubleList.listIterator(DOUBLE_SET_INDEX + 1); // index 3
            assertEquals("Test failed - expected last element was not returned by previous.",
                    new Double(DOUBLE_VALUES[2]), doubleIterator.previous());
            // Test all elements in a list
        ...
        public void testIteratorPreviousIndex() {
            ...
            // Test at beginning of list
            stringIterator = stringList.listIterator(STRING_SET_INDEX); // index 0
            assertEquals("Test failed - Unexpected previous index at beginning of list.",
                    -1, stringIterator.previousIndex());
            // Test in middle of list
            intIterator = intList.listIterator(INT_SET_INDEX); // index 1
            assertEquals("Test failed - Unexpected previous index in middle of list.",
                    (INT_SET_INDEX - 1), intIterator.previousIndex());
            // Test at end of list
            charIterator = charList.listIterator(CHAR_SET_INDEX); // index 3
            assertEquals("Test failed - Unexpected previous index value at end of list.",
                    (CHAR_SET_INDEX - 1), charIterator.previousIndex());
            // Test on an empty list
            doubleList.clear();
            doubleIterator = doubleList.listIterator(); // index 0
            assertEquals("Test failed - Unexpected previous index value on an empty list.",
                    -1, doubleIterator.previousIndex());
        }
        @Test
        public void testIteratorAdd() {
            ...
            assertEquals("Test failed - Incorrect add to beginning of list.",
                    TO_STRING_INSERT[0], stringList.toString());
            // Add to middle of list
            intIterator = intList.listIterator(INT_INSERT_INDEX); // index 1
            intIterator.add(INT_INSERT_VAL);
            assertEquals("Test failed - Incorrect add to middle of list.",
                    TO_STRING_INSERT[1], intList.toString());
            // Add to end of list
            doubleIterator = doubleList.listIterator(DOUBLE_INSERT_INDEX); // index 3
            doubleIterator.add(DOUBLE_INSERT_VAL);
            assertEquals("Test failed - Incorrect add to end of list.",
                    TO_STRING_INSERT[3], doubleList.toString());
            // Add to empty list
            charList.clear();
            charIterator = charList.listIterator();
            charIterator.add(CHAR_VALUES[0]);
            assertEquals("Test failed - Incorrect add to an empty list.",
                    TO_STRING_ADD1[2], charList.toString());
        }
        @Test
        public void testIteratorSet() {
            ...
            assertEquals("Test failed - Unexpected value after setting first element.",
                    TO_STRING_SET[0], stringList.toString());
            ...
            // validate with toString
            assertEquals("Test failed - Unexpected value after setting middle element.",
                    TO_STRING_SET[1], intList.toString());
            ...
            // validate with toString
            assertEquals("Test failed - Unexpected value after setting middle element.",
                    TO_STRING_SET[3], doubleList.toString());
        ...
        public void testIteratorRemove() {
            ...
            // Validate with list's contains method
            assertFalse("Test failed - Element at middle of list was not removed.",
                    intList.contains(INT_VALUES[1]));
            ...
            // Validate with list's contains method
            assertFalse("Test failed - Element at middle of list was not removed.",
                    doubleList.contains(DOUBLE_VALUES[1]));
            ...
            // Validate with list's contains method
            assertFalse("Test failed - Element at end of list was not removed.",
                    charList.contains(CHAR_VALUES[2]));
        }
        @Test
